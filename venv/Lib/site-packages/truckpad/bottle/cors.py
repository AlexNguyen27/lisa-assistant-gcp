...
class CorsPlugin(object):
    ...
    def apply_headers(self, resp, ctx=None):
        ...
        if origin:
            headers = ','.join(self.allowed_headers) if self.allowed_headers else request.get_header('Access-Control-Request-Headers', '*')
            methods = ','.join(self.cors_url_rules.get(ctx.rule, [ctx.method, 'OPTIONS'])) if ctx else '*'
            ...
            resp.add_header('Access-Control-Allow-Credentials', str(self.allow_credentials).lower())
